...
export const generateCaseNumber = async (caseType: string, firmId: string): Promise<string> => {
  ...
  const existing = await db
    ...
    .from(cases)
    .where(and(eq(cases.firmId, firmId), ilike(cases.caseNumber, `${prefix}%`)));
...
export const getAllCases = async (
  ...
  return rows
    ...
    .map((r) => ({
      ...
      client:      { id: r.clientId, name: `${r.clientFirstName} ${r.clientLastName}` },
      assignee:    r.assigneeFirstName
        ? { name: `${r.assigneeFirstName} ${r.assigneeLastName}`, role: r.assigneeRole }
        : null,
    }));
};
