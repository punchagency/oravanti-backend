...
function getPeriodRange(period: Period): {
  ...
  if (period === 'month') {
    ...
    return { startStr: toDateStr(start), endStr: toDateStr(end), label, months: 1 };
  }
  ...
  if (period === 'quarter') {
    ...
    return { startStr: toDateStr(start), endStr: toDateStr(end), label, months: 3 };
  }

  if (period === 'year') {
    const start = new Date(now.getFullYear(), 0, 1);
    const end   = new Date(now.getFullYear(), 11, 31);
    return { startStr: toDateStr(start), endStr: toDateStr(end), label: String(now.getFullYear()), months: 12 };
  }

  return { startStr: null, endStr: null, label: 'All Time', months: 0 };
}

function getPreviousPeriodRange(period: Period): { startStr: string | null; endStr: string | null } {
  const now = new Date();
...
async function fetchHoursByStaff(
  ...
  const rows = await db
    .select({ staffId: timeEntries.staffId, total: sum(timeEntries.hoursWorked) })
    .from(timeEntries)
    .where(
      and(
  ...
  return Object.fromEntries(rows.map((r) => [r.staffId, parseFloat(r.total ?? '0')]));
}
...
async function fetchSkillLevelByStaff(staffIds: string[]): Promise<Record<string, string>> {
  if (staffIds.length === 0) return {};

  const rows = await db
    .select({ staffId: staffCertifications.staffId, level: certifications.level })
    .from(staffCertifications)
    .innerJoin(certifications, eq(certifications.code, staffCertifications.certificationCode))
  ...
  return Object.fromEntries(
    Object.entries(maxLevel).map(([id, order]) => [id, CERT_LEVEL_LABEL[order] ?? 'basic']),
  );
}
...
export const getRevenueAnalytics = async (
  ...
  if (staffList.length === 0) {
    return {
      periodLabel: label,
      summary:     { totalRevenue: 0, totalSalaries: 0, overallROI: 0, totalHours: 0, totalStaff: 0, revenueTrend: null },
      charts:      { revenueVsSalary: [], revenueDistribution: [], efficiencyChart: [] },
      staff:       [],
    };
  }
  ...
  const [hoursByStaff, prevHoursByStaff, skillLevels] = await Promise.all([
    ...
    prevRange.startStr
      ? fetchHoursByStaff(firmId, staffIds, prevRange.startStr, prevRange.endStr)
  ...
  const overallROI     = totalSalaries > 0
  ...
  if (prevRange.startStr) {
    ...
    if (prevRevenue > 0) {
      revenueTrend = Math.round(((totalRevenue - prevRevenue) / prevRevenue) * 1000) / 10;
    }
  }

  const staffWithShare = staffMetrics.map((s) => ({
    ...s,
    revenueShare: totalRevenue > 0 ? Math.round((s.revenue / totalRevenue) * 1000) / 10 : 0,
  }));
