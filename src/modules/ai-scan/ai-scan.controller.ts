...
export class AiScanController {
   ...
   */
  rerunScenario = async (req: Request, res: Response) => {
  ...
  runFullScan = async (_req: Request, res: Response) => {
    const { organizationId, staffId } = getRequestContext();
    const result = await enqueueFullScan(organizationId!, staffId ?? undefined);
