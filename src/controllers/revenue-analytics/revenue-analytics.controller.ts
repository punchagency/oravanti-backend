...
import { getRevenueAnalytics, Period } from '../../services/revenue-analytics/revenue-analytics.service';
...
export const getAnalytics = async (req: AuthRequest, res: Response) => {
  ...
  if (!VALID_PERIODS.includes(period)) {
    res.status(400).json({ message: 'Invalid period. Use: month, quarter, year, all' });
    return;
  }
