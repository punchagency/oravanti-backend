...
describe("proRateSplit", () => {
  it("apportions in proportion to what each side still owes", () => {
    const split = proRateSplit(600, 500, 1500);
    expect(split.trust).toBe(450);
    expect(split.operating).toBe(150);
  });

  it("always sums exactly to the payment, so the check constraint holds", () => {
    // A ratio that does not divide evenly — the remainder must land somewhere
    // rather than vanishing.
    const split = proRateSplit(100, 333, 667);
  ...
  it("attributes to operating when nothing is outstanding", () => {
    ...
    expect(proRateSplit(50, 0, 0)).toEqual({ operating: 50, trust: 0 });
  });

  it("sends the whole payment to the side that owes it", () => {
    expect(proRateSplit(200, 0, 1000)).toEqual({ operating: 0, trust: 200 });
    expect(proRateSplit(200, 1000, 0)).toEqual({ operating: 200, trust: 0 });
  });

  it("never apportions more trust than the payment itself", () => {
    const split = proRateSplit(10, 0, 100000);
